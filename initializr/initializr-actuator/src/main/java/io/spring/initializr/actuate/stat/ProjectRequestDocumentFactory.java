...
public class ProjectRequestDocumentFactory {
	...
	public ProjectRequestDocument createDocument(ProjectRequestEvent event) {
		...
		document.setDeploy(request.getDeploy());
		if (StringUtils.hasText(request.getDeploy())
				&& metadata.getDeploys().get(request.getDeploy()) == null) {
			document.triggerError().setDeploy(true);
		}
