...
public class InitializrMetadataTestBuilder {
	...
	public InitializrMetadataTestBuilder addBasicDefaults() {
		return addDefaultTypes().addDefaultPackagings().addDefaultJavaVersions()
				.addDefaultLanguages().addDefaultBootVersions().addDefaultDeploys();
	}
	...
	public InitializrMetadataTestBuilder addDefaultDeploys() {
		return addDeploy("puppet", true).addDeploy("docker", false);
	}

	public InitializrMetadataTestBuilder addDeploy(String id, boolean defaultValue) {
		this.builder.withCustomizer((it) -> {
			DefaultMetadataElement element = new DefaultMetadataElement();
			element.setId(id);
			element.setName(id);
			element.setDefault(defaultValue);
			it.getDeploys().getContent().add(element);
		});
		return this;
	}
